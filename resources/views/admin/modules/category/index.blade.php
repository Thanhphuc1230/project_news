                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="parent_id" >
                                @foreach($category_selected as $category){
                                    <option selected="" value="{{$category->id_category}}">{{$category->name_cate}}</option>    
                                }
                                @endforeach
   ...
   @foreach ($categories as $cate)     
    ...
    <td>{{ $loop->iteration }}</td>
        ...
        <td>
            ...
            @endphp
               <a  onclick="return confirm('Xác nhận kích hoạt nhãn hàng ?')"  href="{{ route('admin.categories.unactive_categories',['uuid' => $cate->uuid]) }} "class="status_btn" style="background:#FA8072!important">Unactive</a>   
            ...
            <a onclick="return confirm('Xác nhận tắt kích hoạt nhãn hàng ?')"  href=" {{ route('admin.categories.active_categories',['uuid' => $cate->uuid]) }}"class="status_btn">Active</a> 
            @php
                    }
            @endphp
        ...
        <td><div class="action_btns d-flex">
            <a  href="{{ route('admin.categories.edit', ['uuid' => $cate->uuid]) }}" class="action_btn mr_10"> <i class="far fa-edit"></i> </a>
            <a  onclick="return confirm('Xác nhận xóa nhãn hàng ?')"  href="{{ route('admin.categories.destroy', ['uuid' => $cate->uuid]) }}" class="action_btn"> <i class="fas fa-trash"></i> </a>
            </div></td>
