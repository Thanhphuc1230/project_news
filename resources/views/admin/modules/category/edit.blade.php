...
<form action="{{ route('admin.categories.update', ['uuid'=> $category->uuid]) }}" method="post" enctype="multipart/form-data">
    @csrf
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="parent_id">
                @foreach ($category_selected as $category_option)
                    <option value="{{ $category_option->id_category }}" {{ old('parent_id', $category->parent_id) == $category_option->id_category  ? 'selected' : '' }}>
                        {{ $category_option->name_cate }}
                    </option>
                @endforeach
                <option value="0" {{ old('parent_id', $category->parent_id) == 0 ? 'selected' : '' }}>Nhãn hàng mới</option>
            </select>
        </div>
        <div class="mb-3">
            <h6 class="card-subtitle mb-2">Trạng thái</h6>
            <select class="form-select" name="status_cate" >
                <option selected="" value="1" {{ old('status',$category->status_cate ) == 1 ? 'selected' : ''}}>Hiện</option>
                <option value="0" {{ old('status',$category->status_cate ) == 0 ? 'selected' : ''}}>Ẩn</option>
            </select>
        </div>
